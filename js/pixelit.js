...
class pixelit {
  constructor(config = {}) {
    //target for canvas  мишень для холста
    ...
    //hide image elementскрыть элемент изображения
    this.hideFromImg();
   ...
   * @returns {number} limits [0-441.6729559300637] цветовое сходство между цветами, чем меньше, тем лучше
 * @param {array} RGBColor массив целых чисел для создания цвета rgb: [int,int,int,int]
 * @param {array} сравнивает цвет массива целых чисел, чтобы получить цвет rgb: [int,int,int,int]
 * @возвращает {число} ограничений [0-441.6729559300637]
   */
   ...
   *rучитывая фактический цвет, выберите из палитры цветов наиболее подходящий цвет
 * @param {array} Фактический цвет rgb для сравнения [int,int,int]
 * @возвращает {array} приблизительный цвет rgb учитывая фактический цвет, выберите из палитры цветов наиболее приближенный цвет
 * @param {array} Фактический цвет rgb для сравнения [int,int,int]
 * @возвращает {массив} aproximated colorучитывая фактический цвет RGB, и выберите из палитры цветов наиболее подходящий цвет
 * @param {массив} Графический элемент rgb для преобразования [int,int,int]
 * @отображает {массив} отображаемый rgb
   */
  ...
  */
   ...
   * пикселизация на основе @author rogeriopvl <https://github.com/rogeriopvl/8bit>
 * Рисует пикселизированную версию изображения на заданном холсте
   */
  ...
  pixelate() {
    ...
    let scaledW = 72.7;
    let scaledH = 104.5;
    //'50'+42 120.5this.drawto.height * this.scale//this.drawto.width * this.scale

    //создайте временный холст, чтобы создать новую масштабированную копию
    const tempCanvas = document.createElement("canvas");

    // Установите временную ширину / высоту холста и скройте (исправлено обрезание нижней части изображения в большем масштабе)
    ...
    if (this.drawto.width > 900 || this.drawto.height > 900) {
      //fix sclae to pixelate bigger images исправьте sclae для пикселизации больших изображений
      this.scale *= 0.5;
      scaledW = this.drawto.width * this.scale;
      scaledH = this.drawto.height * this.scale;
      //make it big enough to fit сделайте его достаточно большим, чтобы он поместился
    ...
    //configs to pixelate//сграживание пикс
    this.ctx.mozImageSmoothingEnabled = false;
    this.ctx.webkitImageSmoothingEnabled = false;
    this.ctx.imageSmoothingEnabled = false;

    //calculations to remove extra border /вычисления для удаления лишней границы
  ...
  convertGrayscale1() {
    ...
    for (var y = 0; y < imgPixels.height; y++) {
      for (var x = 0; x < imgPixels.width; x++) {
        ...
        imgPixels.data[i] = 1.8*(imgPixels.data[i]-125)+128;
        imgPixels.data[i + 1] = 1.8*(imgPixels.data[i + 1]-128)+158;
        imgPixels.data[i + 2] = 1.8*(imgPixels.data[i + 2]-128)+128;
      }
    }
  ...
  convertGrayscale2() {
    ...
    for (var y = 0; y < imgPixels.height; y++) {
      for (var x = 0; x < imgPixels.width; x++) {
        var i = y * 4 * imgPixels.width + x * 4;
        var avg = (imgPixels.data[i] + imgPixels.data[i + 1] + imgPixels.data[i + 2]) / 3;
        imgPixels.data[i] = 2.1*(imgPixels.data[i]-125)+128;
        imgPixels.data[i + 1] = 2.1*(imgPixels.data[i + 1]-128)+158;
        imgPixels.data[i + 2] = 2.1*(imgPixels.data[i + 2]-128)+128;
      }
    }
    this.ctx.putImageData(imgPixels, 0, 0, 0, 0, imgPixels.width, imgPixels.height);
    return this;
...
//
    ...
    for (var y = 0; y < imgPixels.height; y++) {
      for (var x = 0; x < imgPixels.width; x++) {
        var i = y * 4 * imgPixels.width + x * 4;
        var avg = (imgPixels.data[i] + imgPixels.data[i + 1] + imgPixels.data[i + 2]) / 3;
         imgPixels.data[i] = 2.8*(imgPixels.data[i]-125)+128;
        imgPixels.data[i + 1] = 2.8*(imgPixels.data[i + 1]-128)+158;
        imgPixels.data[i + 2] = 2.8*(imgPixels.data[i + 2]-128)+128;
      }
    }
    this.ctx.putImageData(imgPixels, 0, 0, 0, 0, imgPixels.width, imgPixels.height);
    return this;
   ...
   convertGrayscale4() {
    ...
    for (var y = 0; y < imgPixels.height; y++) {
      for (var x = 0; x < imgPixels.width; x++) {
        var i = y * 4 * imgPixels.width + x * 4;
        var avg = (imgPixels.data[i] + imgPixels.data[i + 1] + imgPixels.data[i + 2]) / 3;
        imgPixels.data[i] = imgPixels.data[i]+20;
        imgPixels.data[i + 1] = imgPixels.data[i + 1]+20;
        imgPixels.data[i + 2] = imgPixels.data[i + 2]+20;
      }
    }
    this.ctx.putImageData(imgPixels, 0, 0, 0, 0, imgPixels.width, imgPixels.height);
    return this;
  ...
  /**
   * converts image to palette using the defined palette or default paletteпреобразует изображение в палитру, используя определенную палитру или палитру по умолчанию
   */
  convertPalette() {
    const w = this.drawto.width;
    const h = this.drawto.height;
  ...
  resizeImage() {
    ...
    //if none defined skip если ничего не определено, пропустите
    ...
    //max height overrides max width максимальная высота переопределяет максимальную ширину
  ...
  draw() {
    ...
    // draw  this.ctx.filter = "brightness(190%)";
  ...
  /**
   тень мозаики работает на хроме не работает в яндексе
